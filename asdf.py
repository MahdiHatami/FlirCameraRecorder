...
class CamGUI(object):
    ...
    def __init__(self):
        ...
        self.mouse_list = self.cam_details[
            'subjects'] if 'subjects' in self.cam_details else []
        self.labview_channel_list = self.cam_details[
            'labview'] if 'labview' in self.cam_details else []
        self.cam_names = ()
        self.output_dir = ()
        for i in range(self.cam_details['cams']):
            self.cam_names = self.cam_names + (
            self.cam_details[str(i)]['name'],)
            self.output_dir = self.output_dir + (
            self.cam_details[str(i)]['output_dir'],)
    ...
    def set_up_vid(self):
        ...
        if len(self.vid_out) > 0:
            ...
            Label(vid_open_window,
                  text="Video is currently open! \nPlease release the current "
                       "video (click 'Save Video', even if no frames have "
                       "been recorded) before setting up a new one.").pack()
            Button(vid_open_window, text="Ok",
        ...
        if len(self.cam) == 0:
            ...
            for i in range(len(self.cam)):
                ...
                self.base_name.append(cam_name_nospace[
                                          i] + '_' + self.subject.get() + '_'
                                      + date + '_')
                self.vid_file.append(os.path.normpath(
                    self.out_dir + '/' + self.base_name[
                        i] + self.attempt.get() + '.avi'))

                # check if file exists, ask to overwrite or change attempt
                # number if it does
                if i == 0:
                        ...
                        Label(self.ask_overwrite,
                              text="File already exists with attempt number = "
                                   "" + self.attempt.get() + ".\nWould you "
                                                             ...
                                                             "overwrite the "
                                                             "file? ").pack()
                        Button(self.ask_overwrite, text="Overwrite",
                ...
                self.ts_file[i] = self.ts_file[i].replace(cam_name_nospace[i],
                                                          'TIMESTAMPS_' +
                                                          cam_name_nospace[i])

                self.current_file_label[
                    'text'] = self.subject.get() + '_' + date + '_' + \
                              self.attempt.get()

                # create video writer
                dim = self.cam[i].get_image_dimensions()
                if len(self.vid_out) >= i + 1:
                    self.vid_out[i] = cv2.VideoWriter(self.vid_file[i],
                                                      cv2.VideoWriter_fourcc(
                                                          *'DIVX'),
                                                      int(self.fps.get()), dim)
                else:
                    self.vid_out.append(cv2.VideoWriter(self.vid_file[i],
                                                        cv2.VideoWriter_fourcc(
                                                            *'DIVX'),
                                                        int(self.fps.get()),
                                                        dim))
    ...
    def selectCams(self):
        ...
        # number of cameras
        Label(select_cams_window, text="How many cameras?").grid(sticky="w",
                                                                 row=0,
                                                                 column=0)
        self.number_of_cams = StringVar(value="1")
        self.number_of_cams_entry = Entry(select_cams_window,
                                          textvariable=self.number_of_cams).grid(
            sticky="nsew", row=0, column=1)
        Button(select_cams_window, text="Set Cameras",
               command=select_cams_window.quit).grid(sticky="nsew", row=1,
    ...
    def createGUI(self):
        ...
        # set up video
        Button(self.window, text="Set Up Video", command=self.set_up_vid).grid(
