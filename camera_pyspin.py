...
while True:
    ...
    if image_bit == 16:
        test_image = cam.GetNextImage().Convert(
            PySpin.PixelFormat_Mono16).GetNDArray()
    else:
    ...
    print('Image value: min = {}, max = {}'.format(np.min(test_image),
                                                   np.max(test_image)))
    answer = input(
        '''Enter 'y' to accept current exposure time {:d}ms, or enter a new 
        exposure time in millisecond: '''.format(
            ExposureTime))
    if answer.lower() == 'y':
        plt.close('all')
        cam.EndAcquisition()
...
def save_img(image):
    # convert PySpin image object into ND array, rescale the image,
    # save small jpg image and the ND array.
...
try:
    input(
        'Images will be saved to folder {}\nPress enter to start. Press '
    ...
    while True:
        ...
        # multithreads to accelerate saving and avoid filling up the buff of
        # the camera
...
print('AcquisitionResultingFrameRate: {:.2f}FPS'.format(
    cam.AcquisitionResultingFrameRate()))
